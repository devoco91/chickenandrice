...
const money = (n=0) => new Intl.NumberFormat('en-NG',{style:'currency',currency:'NGN'}).format(Number(n||0))
...
export default function SuccessPage() {
  ...
  const progressPct = useMemo(() => Math.min(100, Math.max(0, ((totalSeconds - remaining) / totalSeconds) * 100)), [remaining, totalSeconds])
    ...
    <>
      ...
      <div className="relative min-h-[100dvh] bg-slate-950 text-slate-100">
        <main className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 mt-10 py-10 sm:py-16">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mx-auto w-full rounded-2xl border border-white bg-slate-900 p-6 sm:p-8">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-emerald-600/15">
              <CheckCircle2 className="h-10 w-10 text-emerald-400" />
            </div>

            <h1 className="text-3xl font-semibold tracking-tight text-center">Order Successful!</h1>
            ...
            <div className="mt-6 flex flex-col items-center justify-center gap-3">
              <div className="inline-flex items-center gap-2 rounded-full border border-white bg-black px-3 py-1.5 text-sm">
                ...
                <button onClick={handleCopyRef} className="inline-flex items-center gap-1 rounded-md border border-white bg-black px-2 py-1 text-xs hover:bg-slate-800">
            ...
            {summary && (
              <div className="mt-8 rounded-xl border border-white/60">
                <div className="px-4 py-3 border-b border-white/60 text-sm flex items-center justify-between">
                  ...
                  <span className="font-medium capitalize">{summary.deliveryMethod || 'pickup'}</span>
                </div>
                <div className="divide-y divide-white/50">
                  {summary.items?.map((it) => (
                    <div key={`${it.foodId}-${it.name}`} className="px-4 py-3 text-sm flex items-center justify-between">
                      <span className="opacity-90">{it.name} × {it.quantity}</span>
            ...
            <div className="mt-10 flex items-center justify-center">
              <Link href="/" className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-600 px-5 py-2 text-sm font-medium text-white shadow hover:bg-emerald-700">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-2 text-xs text-slate-300">
              <button onClick={() => window.print()} className="inline-flex items-center gap-1 rounded-md border border-white bg-black px-2.5 py-1.5 hover:bg-slate-800" title="Print receipt">
                <Printer className="h-3.5 w-3.5" /> Print
              </button>
              <button onClick={handleShare} className="inline-flex items-center gap-1 rounded-md border border-white bg-black px-2.5 py-1.5 hover:bg-slate-800" title="Share order status">
                <Share2 className="h-3.5 w-3.5" /> {shared ? 'Copied' : 'Share'}
              </button>
            </div>

            <p className="mt-4 text-xs text-slate-400 text-center">Keep this page open to see your live countdown.</p>
          </motion.div>
        </main>
      </div>
    </>
