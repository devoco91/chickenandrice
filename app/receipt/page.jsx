...
export default function ReceiptPage() {
  ...
  const [orderType, setOrderType] = useState("instore") // <-- reads Chowdeck flag

  // Read values set by Summary page / Order page
  useEffect(() => {
    if (typeof window !== "undefined") {
      const pm = sessionStorage.getItem("paymentMode") || ""
      const oid = sessionStorage.getItem("lastOrderId") || ""
      const ot = sessionStorage.getItem("orderType") || "instore"
      if (pm) setPaymentMode(pm)
      if (oid) setOrderId(oid)
      setOrderType(ot === "chowdeck" ? "chowdeck" : "instore")
    }
  }, [])
  ...
  const handlePrint = useReactToPrint({
    ...
    onAfterPrint: () => {
      dispatch(clearCart2())
      router.replace("/order") // prevents back navigation to the receipt after printing
    },
  })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-yellow-50">
      <div className="max-w-3xl mx-auto px-6 md:px-10 pb-28">
      ...
      {/* PRINT-ONLY: Single copy (no "Customer/Merchant", shows CHOWDECK only for chowdeck) */}
      <div className="hidden print:block">
        <div ref={componentRef}>
          <div className="bg-white/90 backdrop-blur rounded-2xl border border-gray-200 shadow-lg p-6 print:w-[80mm] print:rounded-none print:border-0 print:shadow-none print:p-3">
            <div className="print:font-mono">
              {/* CHOWDECK banner (print only, when applicable) */}
              {orderType === "chowdeck" && (
                <p className="text-center font-black text-2xl tracking-widest uppercase mb-2">
                  CHOWDECK
                </p>
              )}
              ...
              <div>
                ...
                {cart.map((item) => {
                  ...
                  return (
                    <div
                      key={`${item._id}-${item.category}-single`}
                      className="grid grid-cols-12 items-center text-sm py-2 border-b border-dashed last:border-b-0 print:text-[13px]"
                    >
                      <div className="col-span-7 pr-2">
                        <p className="font-medium text-gray-900 truncate">{item.name}</p>
