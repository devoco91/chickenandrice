// app/api/delivery/login/route.js
...
// copy request headers, avoid hop-by-hop, force identity (no gzip)
function copyReqHeaders(req) {
  ...
  h.set("accept-encoding", "identity"); // ← stop compressed bytes coming through
...
async function proxy(method, req) {
  ...
  const res = await fetch(`${BACKEND}/api/delivery/login`, {
    method,
    headers,
    body,
    cache: "no-store",
...
export async function POST(req) {
  try {
    ...
    console.error("Proxy POST /api/delivery/login failed:", e);
