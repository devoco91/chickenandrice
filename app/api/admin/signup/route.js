...
async function proxy(method, req) {
  ...
  if (method !== "GET" && method !== "HEAD") {
    ...
    if (ct.includes("multipart/form-data")) {
      const fd = await req.formData(); body = fd; headers.delete("content-type");
    } else if (ct.includes("application/json")) {
      body = await req.text(); headers.set("content-type", "application/json");
    } else { body = await req.text(); }
  }
  const res = await fetch(`${BACKEND}/api/admin/signup`, {
    method, headers, body, cache: "no-store", redirect: "follow",
  });
  return new Response(res.body, { status: res.status, headers: copyResHeaders(res.headers) });
}
export async function POST(req) {
  try { return await proxy("POST", req); }
  catch (e) { console.error("Proxy POST /api/admin/signup failed:", e);
